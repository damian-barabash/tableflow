...
import { fetchDraftOrPublished, flatten, publish, saveDraft, type Overrides } from '../lib/content'
import { createEngine, type Changes } from '../editor/engine'
import './edit-mod.css'
...
/** /edit-mod — login gate → visual on-page editor for every page and language. */
  ...
  return (
    <div className="ed-gate">
      <div className="ed-login card">
        ...
        <p className="sub">Konto <b>{email}</b> nie ma roli moderatora.</p>
        <button className="btn btn--ghost" onClick={onLogout}>Wyloguj</button>
      </div>
    </div>
  )
}
...
function Editor({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  const [composite, setComposite] = useState<{ parts: { path: string; value: string }[]; apply: (v: string[]) => void } | null>(null)
  const [count, setCount] = useState(0)
  const root = useRef<HTMLDivElement>(null)
  const engine = useRef<ReturnType<typeof createEngine> | null>(null)

  useEffect(() => { setEditMode(true); document.body.classList.add('editing'); return () => { setEditMode(false); document.body.classList.remove('editing'); setEditorOverrides(locale, null) } }, []) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // load draft (or published) overrides for this locale into the provider
  useEffect(() => {
    let alive = true
    setStatus('loading')
    fetchDraftOrPublished(locale).then(({ data, status }) => { if (!alive) return; setEditorOverrides(locale, data); setSource(status); setChanges({}); setStatus('clean') })
    return () => { alive = false }
  }, [locale, setEditorOverrides])

  // (re)create the engine whenever page / locale / loaded overrides change
  ...
  useEffect(() => {
    if (status === 'loading' || !root.current) return
    engine.current?.destroy()
    const eng = createEngine({
      root: root.current, values,
      onChange: ch => { setChanges(ch); setStatus('dirty') },
      onFocusPath: setFocusPath,
      onComposite: req => setComposite({ parts: req.parts, apply: req.apply }),
    })
    engine.current = eng
    const id = window.setTimeout(() => setCount(eng.count()), 400)
    return () => { clearTimeout(id); eng.destroy() }
  }, [values, page, status === 'loading']) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const merged = useCallback((): Overrides => {
    ...
    for (const [k, v] of Object.entries(changes)) { if (values.has(k) && v === flatten(base).get(k)) delete out[k]; else out[k] = v }
    return out
  }, [overrides, changes, values, base])

  const doSave = async () => {
    setStatus('saving')
    try { const data = merged(); await saveDraft(locale, data); setEditorOverrides(locale, data); setChanges({}); setSource('draft'); setStatus('saved-draft') }
    catch { setStatus('error') }
  }
  const doPublish = async () => {
    setStatus('saving')
    try { const data = merged(); await publish(locale, data); setEditorOverrides(locale, data); setChanges({}); setSource('published'); setStatus('published') }
    catch { setStatus('error') }
  }
  const discard = () => { if (!Object.keys(changes).length || confirm('Odrzucić niezapisane zmiany?')) window.location.reload() }
  const changeLocale = (l: Locale) => { if (Object.keys(changes).length && !confirm('Masz niezapisane zmiany. Zmienić język bez zapisu?')) return; setLangOpen(false); void setLocale(l, { animate: false, persist: false }) }
    ...
    <div className="ed">
      ...
      <div className="ed-bar" role="toolbar" aria-label="Edytor">
        <div className="ed-bar__group">
          <button className="ed-btn ed-btn--page" onClick={() => { setPickerOpen(v => !v); setLangOpen(false) }} aria-expanded={pickerOpen}>
          ...
          <button className="ed-btn" onClick={() => { setLangOpen(v => !v); setPickerOpen(false) }} aria-expanded={langOpen}><I.globe width={15} height={15} /><span>{LOCALE_NAMES[locale]}</span></button>
          <AnimatePresence>
            {langOpen && (
              <motion.ul className="ed-pop ed-pop--lang" initial={{ opacity: 0, y: 8, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: .98 }} transition={{ duration: .22 }}>
                <li className="ed-pop__title">Język treści</li>
                {LOCALES.map(l => <li key={l}><button className={l === locale ? 'is-active' : ''} onClick={() => changeLocale(l)}><span>{LOCALE_NAMES[l]}</span>{l === locale && <I.check width={14} height={14} />}</button></li>)}
              </motion.ul>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="ed-bar__group ed-bar__actions">
          ...
          <button className="btn btn--ghost btn--sm" onClick={doSave} disabled={status === 'saving' || status === 'loading'}>Zapisz jako draft</button>
          <button className="btn btn--primary btn--sm" onClick={doPublish} disabled={status === 'saving' || status === 'loading'}>Opublikuj <I.arrowUp width={14} height={14} /></button>
      ...
      <AnimatePresence>
        {composite && (
          <motion.div className="ed-modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setComposite(null)}>
            <motion.form className="ed-pop ed-pop--multi card" onClick={e => e.stopPropagation()} initial={{ y: 14, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 8, scale: .98 }}
              onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); composite.apply(composite.parts.map((_, i) => String(f.get(`p${i}`) ?? ''))); setComposite(null) }}>
              <div className="ed-pop__title">Nagłówek składa się z {composite.parts.length} części</div>
              {composite.parts.map((p, i) => <label key={p.path} className="ed-field"><span>{i + 1} · <code>{p.path}</code></span><input className="input" name={`p${i}`} defaultValue={p.value} /></label>)}
              <div className="row" style={{ justifyContent: 'flex-end', gap: 8 }}>
                <button type="button" className="btn btn--ghost btn--sm" onClick={() => setComposite(null)}>Anuluj</button>
                <button type="submit" className="btn btn--primary btn--sm">Zastosuj</button>
              </div>
            </motion.form>
          </motion.div>
        )}
